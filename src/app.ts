...
<footer><section><h2>使い方</h2><ol><li>キーワードを入力するか一覧から形を選びます。</li><li>幅・高さ・厚みをmm単位で設定します。</li><li>必要ならキーホルダー用タブを付けます。</li><li>3Dプレビューを確認してSTLを保存します。</li></ol></section><section><h2>注意事項</h2><p>AI画像生成、外部画像検索、任意画像アップロードは行いません。対応するのは登録済みのオリジナルシルエットだけです。</p></section><section><h2>ライセンス</h2><p>初期SVGはこのプロジェクト用に作成した単純なオリジナル形状です。第三者アイコン素材は含みません。</p></section></footer>`;
  ...
  const update = (): void => {
    ...
    root.querySelector('#sizeInfo')!.textContent = `現在の設定：幅${settings.width}mm × 高さ${settings.height}mm × 厚み${settings.depth}mm`;
